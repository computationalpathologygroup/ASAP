...
namespace ASAP::Data
{
	...
	size_t FilelistDataAcquisition::GetImageThumbnailFile(const std::string& image_index, const std::function<void(boost::filesystem::path)>& receiver, const std::function<void(float)> observer)
	{
		receiver(boost::filesystem::path(*m_images_.At(std::stoi(image_index), { "location" })[0]));
		return 0;
	}

	size_t FilelistDataAcquisition::GetImageFile(const std::string& image_index, const std::function<void(boost::filesystem::path)>& receiver, const std::function<void(float)> observer)
	{
		receiver(boost::filesystem::path(*m_images_.At(std::stoi(image_index), { "location" })[0]));
		return 0;
	}
