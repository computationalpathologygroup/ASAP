...
namespace ASAP::Data
{
	...
	class WorklistDataAcquisitionInterface
	{
		public:
			...
			/// <return>The task id, which can be used to cancel asynchronous tasks.</return>
			virtual size_t GetImageThumbnailFile(const std::string& image_index, const std::function<void(boost::filesystem::path)>& receiver, const std::function<void(float)> observer) = 0;
			/// <summary>
			...
			/// <return>The task id, which can be used to cancel asynchronous tasks.</return>
			virtual size_t GetImageFile(const std::string& image_index, const std::function<void(boost::filesystem::path)>& receiver, const std::function<void(float)> observer) = 0;
